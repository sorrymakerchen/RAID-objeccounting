...
def test_anomaly_detection(args, cls, model, net, masking, all_cls_prototypes, knn_metric, knn_index, knn_neighbors,
    ...
    all_image_paths = []
    label = []
    # data_root = "/home/ZYP/datasets/mvtec/bottle"
    net.eval()
    net.to(args.device)
            ...
            for sub_root, sub_dirs, sub_files in os.walk(root):
                ...
                for file in sub_files:
                    if Path(file).suffix.lower() in image_extensions:
                        ...
                        mask_file = f"{stem}_mask{suffix}"  # 如 015.png -> 015_mask.png
    ...
    with torch.no_grad():
        for image_test_path, cls_label, mask_path in all_image_paths:
            ...
            if mask_path:
                ...
                mask_tensor = torch.from_numpy(mask).float().to(args.device)  # 归一化前
            ...
            if knn_metric == "L2":
                ...
                if pred_label == "screw" or pred_label == "capsule" or pred_label == "wood":
                    knn_index[pred_label].nprobe = 20
                else:
                    knn_index[pred_label].nprobe = 5
