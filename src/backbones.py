...
class DINOv2Wrapper(VisionTransformerWrapper):
    ...
    def prepare_image(self, img):
        ...
        grid_size = ((cropped_height // self.model.patch_size) * 2, (cropped_width // self.model.patch_size) *2)
    ...
    def extract_features(self, image_tensor):
        with torch.inference_mode():
            ...
            # tokens = self.model.get_intermediate_layers(image_batch, return_class_token=True)[0].squeeze() #因为没有传 n 参数，所以默认 n=1，返回的是一个长度为 1 的列表，最后一层的输出
            feature, clstoken = self.model.get_intermediate_layers(image_tensor, return_class_token=True)[0]
            ...
            #         norm: bool = True,  # 👈 默认为 True
            # ) -> List[torch.Tensor]:
            # print(tokens[0,:].shape)
            if flag == 1:
                return feature[0].squeeze().cpu().numpy(), clstoken.cpu().numpy()
            else:
                return feature.cpu().numpy(), clstoken.cpu().numpy()
    ...
    # compute_background_mask 是一个基于 PCA + 自适应方向 + 形态学处理的无监督背景过滤方法，能有效去除 DINO 特征中的背景噪声，提升异常检测精度。
    # [CLS]token的注意力图（attention map）自动聚焦在图像中的主要物体上！
    def compute_background_mask(self, img_features, grid_size, threshold=10, masking_type=False, kernel_size=3,
                                border=0.2):
        # Kernel size for morphological operations should be odd

        # img_features[1024, 384]
        ...
        if masking_type == True:
            ...
            mask = np.ones_like(first_pc, dtype=bool)  # 给每个patch确定是否是前景
        return mask.squeeze()
