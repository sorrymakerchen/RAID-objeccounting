...
def run_anomaly_detection(
    ...
    for object_name in objects:
        ...
        if n_ref_samples == -1:
            ...
            # few-shot setting, pick samples in deterministic fashion according to seed，随机选择参考图像
            #img_ref_samples = sorted(os.listdir(img_ref_folder))[seed*n_ref_samples:(seed + 50)*n_ref_samples]
            all_imgs = sorted(os.listdir(img_ref_folder))
            if object_name == "pcb1" or object_name == "pcb2" or object_name == "pcb3" or  object_name == "macaroni2":
                img_ref_samples = random.sample(all_imgs, 500) #500
            else:
                img_ref_samples = random.sample(all_imgs, 100)#100
